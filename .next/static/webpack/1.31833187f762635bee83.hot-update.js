...
var Demo = /*#__PURE__*/function (_Component) {
  ...
  function Demo(props) {
    ...
    Object(_babel_runtime_helpers_esm_defineProperty__WEBPACK_IMPORTED_MODULE_7__["default"])(Object(_babel_runtime_helpers_esm_assertThisInitialized__WEBPACK_IMPORTED_MODULE_3__["default"])(_this), "getInterpolatedData", function (event) {
      event.preventDefault();
      console.log("Interpolation fucntion" + JSON.stringify(_this.state.result)); //const duration = JSON.parse(JSON.stringify(this.state.result.duration))
      ...
      for (var annotation = 0; annotation < temp.annotations.length; annotation++) {
        for (var incident = 0; incident < temp.annotations[annotation].trajectories.length; incident++) {
      ...
      for (var _annotation = 0; _annotation < temp.annotations.length; _annotation++) {
        for (var _incident = 0; _incident < temp.annotations[_annotation].trajectories.length; _incident++) {
          temp.annotations[_annotation].trajectories[_incident].label = temp.annotations[_annotation].label;
        }
      }

      for (var i = 0; i < interpolatedResult.annotations.length; i++) {
        var annotIncidentLength = interpolatedResult.annotations[i].trajectories.length;
        ...
        if (annotIncidentLength === 1) {
          ...
          for (var j = 0; j < annotIncidentLength; j++) {
            if (j != annotIncidentLength - 1 && (interpolatedResult.annotations[i].trajectories[j + 1].status == "Re-appear" || interpolatedResult.annotations[i].trajectories[j + 1].status == "" || interpolatedResult.annotations[i].trajectories[j + 1].status == "Split")) {
              var source = interpolatedResult.annotations[i].trajectories[j];
              var target = interpolatedResult.annotations[i].trajectories[j + 1];
            ...
            if (j == annotIncidentLength - 1 && (interpolatedResult.annotations[i].trajectories[j].status == "Re-appear" || interpolatedResult.annotations[i].trajectories[j].status == "" || interpolatedResult.annotations[i].trajectories[j + 1].status == "Split")) {
              var _source = interpolatedResult.annotations[i].trajectories[j];
              var _target = interpolatedResult.annotations[i].trajectories[j];
      ...
      for (var x = 0; x < temp.annotations.length; x++) {
        for (var y = 0; y < temp.annotations[x].trajectories.length; y++) {
...
//# sourceMappingURL=1.31833187f762635bee83.hot-update.js.map